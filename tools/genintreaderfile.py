...
def _func_name(size, at, is_le, is_signed):
    ...
    return 'readInt{}{}{}At{}'.format(sign, bo, size, at)
...
def _gen_algo_le(size, at, bytes):
    ...
    if bytes > 1:
        ...
        print('    res <<= {};'.format(8 - at))

    if at == 0:
        # avoid useless shift by 0
        print('    res |= buf[0];')
    else:
        print('    res |= (buf[0] >> {});'.format(at))
...
def _gen_algo_be(size, at, bytes):
    ...
    if bytes > 1:
        ...
        print('    res <<= {};'.format(8 - sr_len))

    if sr_len == 0:
        # avoid useless shift by 0
        print('    res |= buf[{}];'.format(top))
    else:
        print('    res |= (buf[{}] >> {});'.format(top, sr_len))


def _gen_algo(size, at, is_le, is_signed):
    init = 'static {} {}('.format(_ret_type(is_signed),
                                  _func_name(size, at, is_le, is_signed))
    print(init, end='')
...
def _gen_table(is_le, is_signed):
    name = 'readInt{}{}Funcs'.format('Le' if is_le else 'Be',
                                     'Signed' if is_signed else 'Unsigned')
    print('static {} (*{}[])(const std::uint8_t *) = {{'.format(_ret_type(is_signed),
                                                                name))

    for size in range(1, 65):
        for at in range(8):
            print('    {},'.format(_func_name(size, at, is_le, is_signed)))
 ...
 */''')
    print()
    print('#ifndef _YACTFR_INT_READER_HPP')
    print('#define _YACTFR_INT_READER_HPP')
    print()
    print('#include <cstdint>')
    print('#include <cassert>')
    print('#include <cstring>')
    print()
    print('namespace yactfr {')
    ...
    print('} // namespace yactfr')
    print()
    print('#endif // _YACTFR_INT_READER_HPP')

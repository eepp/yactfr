...
int main()
{
    ...
    while (it != std::end(seq)) {
        ...
        yactfr::PacketSequenceIterator newIt {std::move(it)};

        if (it != std::end(seq)) {
            std::cerr << "Moved packet sequence iterator is not set to end of packet sequence.\n";
            return 1;
        }
    ...
    yactfr::PacketSequenceIterator it2 {std::move(endIt)};

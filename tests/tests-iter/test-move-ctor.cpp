...
int main()
{
    ...
    while (it != std::end(seq)) {
        ...
        yactfr::ElementSequenceIterator newIt {std::move(it)};

        if (it != std::end(seq)) {
            std::cerr << "Moved element sequence iterator is not set to end of element sequence.\n";
            return 1;
        }

        // copy (we're testing the move constructor, not the move assignment)
        it = newIt;
        ++it;
    }
    ...
    // move-construct end iterator
    auto endIt = std::end(seq);
    yactfr::ElementSequenceIterator it2 {std::move(endIt)};

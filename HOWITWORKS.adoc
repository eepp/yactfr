...
Trace type
  ...
  Data stream types (0 or more)
    ...
    Event header type (optional)
    Event common context type (optional)
...
* A 4-bit unsigned integer data type is the set of the integer values
  from 0 to 31.
...
The `traceFromMetadataText()` function does exactly that. This is a
templated function which accepts two character iterators (beginning and
end) to synthesize a trace object out of the parsed text.

A trace object has:

* A trace type object.
* A trace environment object.
...
`traceFromMetadataText()` requires a _textual_ (non-packetized) version
...
* `internal::ReadSIntInstr`
* `internal::ReadFloatInstr`
* `internal::ReadStrInstr`
...
subprocedure is usually an `internal::EndReadCompoundInstr` instruction,
...
* `internal::SetDsIdInstr` follows a "`read integer`" (or "`read
  enumeration`") instruction and indicates to the VM to set the current
...
* `internal::UpdateClkValInstr` indicates to the VM to update the value
  of a specific data stream clock with the value (or partial value) of
  the last decoded integer.
...
dynamic array or the selector of a variant, as the
`internal::BeginReadDynArrayInstr`, `internal::BeginReadVarUSelInstr`,
and `internal::BeginReadVarSSelInstr` instructions contain a numeric
...
procedures and to assign appropriate positions to link "`read dynamic
array`" and "`begin read variant`" instructions to their length/selector
values.
...
* Array of data stream clock values.
...
`internal::BeginReadStaticArrayInstr` requires the VM to start reading a
...
instruction to execute to read a single array element. It doesn't
contain register decrementation, comparison, and jump instructions like
you would find in a typical assembly loop. Instead, the state of the VM
is changed (to `VmState::EXEC_ARRAY_INSTR`) so that it knows that it's
...
least aligned on a byte and have lengths of 8, 16, 32, or 64, using the
inline functions in `std-int-reader.hpp`. Those use `std::memcpy()` with
a length known at build time and `Boost.Endian` which generate efficient
CPU instructions the last time I checked. Those integers are typically
the most commonly found in a data stream.
...
integers up to a length of 64{nbsp}bits. Function tables are created
to select:

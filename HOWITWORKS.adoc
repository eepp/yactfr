...
use case is probably to get one out of a standard TSDL metadata document
...
`std::string` object to synthesize a pair of trace type and trace
environment objects out of the parsed text.
...
block in the `metadata` file of a CTF{nbsp}1.8 trace as each trace has

...
When you build an element sequence, you need to pass a shared trace type
and a shared data source factory. Each iterator created by the element
...
a `ElementSequenceIteratorPosition` when calling
`internal::Vm::savePosition()` (called from
`ElementSequenceIterator::savePosition()`).
...
coupled. The VM knows its owning iterator because, when it executes one
or more instructions and the current element changes, it sets the
iterator's `_curElement` member to the address of one of its already
allocated and filled elements (located in the VM position object). It
also sets the iterator's offset and mark.

The element sequence iterator's _mark_ is the index of the current
...
passed-the-end iterator by `ElementSequence::end()`. However, all
iterators contain:

* The shared trace type of its creating element sequence.
* The shared data source factory of its creating element sequence.
...
call `ElementSequenceIterator::seekPacket()`, the iterator's VM's
position and buffers are reset. There can't be any validation that this
...
avoid having too much active data sources, yactfr offers the element
...
An element sequence iterator position does not contain any data source.
It contains:

...
* TSDL: Doesn't support relative dynamic array type lengths and variant
  type selectors in data type aliases (or named structure/variant type)
  which target structure member types outside this data type alias.
+
...
int main(const int argc, const char * const argv[])
{
    ...
    for (auto& elem : seq) {
        if (elem.kind() == yactfr::Element::Kind::EVENT_RECORD_TYPE) {
            auto& ertElem = static_cast<const yactfr::EventRecordTypeElement&>(elem);

            // the name of an event record type is optional
            if (ertElem.eventRecordType().name()) {
                std::cout << *ertElem.eventRecordType().name() << std::endl;
            }
        }
    }
}
...
int main(const int argc, const char * const argv[])
{
    ...
    for (auto it = seq.begin(); it != endIt; ++it) {
        if (it->kind() == yactfr::Element::Kind::EVENT_RECORD_TYPE) {
            auto& ertElem = static_cast<const yactfr::EventRecordTypeElement&>(*it);

            // the name of an event record type is optional
            if (ertElem.eventRecordType().name() &&
                    *ertElem.eventRecordType().name() == "sched_switch") {
                std::cout << "---" << std::endl;
        ...
        if (inSchedSwitchEventRecord && it->kind() == yactfr::Element::Kind::SIGNED_INTEGER) {
            std::cout << it.offset() << ": ";

            auto& intElem = static_cast<const yactfr::SignedIntegerElement&>(*it);
...
int main(const int argc, const char * const argv[])
{
    ...
    for (auto it = seq.begin(); it != endIt; ++it) {
        ...
        case yactfr::Element::Kind::STRING_BEGINNING:
        case yactfr::Element::Kind::STRUCTURE_BEGINNING:
        case yactfr::Element::Kind::STATIC_ARRAY_BEGINNING:
        case yactfr::Element::Kind::STATIC_TEXT_ARRAY_BEGINNING:
        case yactfr::Element::Kind::DYNAMIC_ARRAY_BEGINNING:
        case yactfr::Element::Kind::DYNAMIC_TEXT_ARRAY_BEGINNING:
...
after the decoder reads the expected packet total length unsigned
integer from the packet context. Then, we make the iterator seek the
next packet directly.

Note that this example doesn't work if the packet context type does not
contain an expected packet total length unsigned integer, in which case
the data stream _must_ contain a single packet. This could be detected
by inspecting the metadata (trace type) and using the size of the whole
data stream file as the unique packet total length.
...
int main(const int argc, const char * const argv[])
{
    ...
    while (it != endIt) {
        if (it->kind() == yactfr::Element::Kind::PACKET_BEGINNING) {
            // save packet beginning offset
            curPktOffset = it.offset();
        } else if (it->kind() == yactfr::Element::Kind::EXPECTED_PACKET_TOTAL_LENGTH) {
            // this element indicates the expected total length of the current packet
            auto& elem = static_cast<const yactfr::ExpectedPacketTotalLengthElement&>(*it);

            std::cout << "Packet #" << curPktNumber << ":    " <<
                         "Offset: " << std::setw(10) << curPktOffset << "    " <<
                         "Size: " << std::setw(10) << elem.expectedLength() <<
                         std::endl;
            ++curPktNumber;
             ...
             */
            it.seekPacket((curPktOffset + elem.expectedLength()) / 8);
            continue;
        }

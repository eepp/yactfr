...
beginning/end of packet, beginning/end of event record, beginning/end of
...
* **If you build the API documentation**: https://www.doxygen.nl/[Doxygen]
...
----
$ make tests
$ YACTFR_BINARY_DIR=$(pwd) pytest -n logical ../tests
----
...
. The path to a data stream file of the same trace (required).
...
int main(const int argc, const char * const argv[])
{
    ...
    for (auto it = seq.begin(); it != endIt; ++it) {
        if (it->isEventRecordInfoElement()) {
            ...
            continue;
        } else if (it->isEventRecordEndElement()) {
            // leaving the event record: clear the flag
            inSchedSwitchEventRecord = false;
            continue;
        }
...
int main(const int argc, const char * const argv[])
{
    ...
    while (it != endIt) {
        if (it->isPacketBeginningElement()) {
            ...
            std::cout << "Packet #" << curPktNumber << ":    " <<
                         "Offset: " << std::setw(10) << curPktOffset << "    " <<
                         "Length: " << std::setw(10) << *elem.expectedTotalLength() <<
                         std::endl;
            ++curPktNumber;

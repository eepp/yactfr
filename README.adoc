...
parallelize the testing. You need to set the `YACTFR_BINARY_DIR` to the
build directory (absolute path), for example:

.Run the yactfr tests in parallel (eight processes) from the root directory of the source tree.
----
YACTFR_BINARY_DIR=$(pwd)/build py.test -n8
----
...
int main(const int argc, const char * const argv[])
{
    ...
    for (auto it = seq.begin(); it != endIt; ++it) {
        ...
        case yactfr::Element::Kind::VARIANT_WITH_SIGNED_INTEGER_SELECTOR_BEGINNING:
        case yactfr::Element::Kind::VARIANT_WITH_UNSIGNED_INTEGER_SELECTOR_BEGINNING:
